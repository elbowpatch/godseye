// Shared CCTV proxy helpers. Ported from vite.config.js's cctvProxy() and
// its surrounding source-loader functions, adapted for Vercel serverless
// functions:
//  - The merged source catalog now lives in KV (api/_lib/cache.js) instead
//    of a module-scope Map, with the same 15-minute TTL + serve-stale shape.
//  - Per-camera health tracking is process-memory only (best-effort — it
//    resets whenever a serverless instance cold-starts); it's diagnostic
//    data, not something the app depends on functioning correctly.
...
const CCTV_SOURCE_CACHE_TTL_S = 15 * 60;
const CATALOG_KEY = 'cctv:sources';
...
const CALTRANS_CCTV_URL = (district) => `https://cwwp2.dot.ca.gov/data/d${district}/cctv/cctvStatusD${String(district).padStart(2, '0')}.json`;
...
function loadSourcesFromFile() {
  ...
  const resolved = path.isAbsolute(sourceFile) ? sourceFile : path.resolve(process.cwd(), sourceFile);
  try {
...
function extractAustinHeading(record) {
  ...
  const nameProbe = [record.camera_name, record.location_name, record.intersection_name, record.location, record.cross_street, record.description, record.name].filter(Boolean).join(' ');
...
function prioritizeSources(cameras, maxCount, anchors) {
  ...
  const scored = list.map((camera, idx) => {
    ...
    const distKm = Number.isFinite(lat) && Number.isFinite(lon) ? Math.min(...anchorList.map((a) => haversineKm(lat, lon, a.lat, a.lon))) : Number.POSITIVE_INFINITY;
...
async function loadAustinSourcesFromOpenData() {
  ...
  try {
    const resp = await fetchWithTimeout(endpoint, { headers: { Accept: 'application/json' } }, CCTV_SOURCE_FETCH_TIMEOUT_MS);
    ...
    for (const row of rows) {
      ...
      cameras.push({
        id: cameraId, name: extractAustinName(record, cameraId), city: 'Austin', cityId: 'austin',
        provider: 'Austin Transportation & Public Works', lat, lon, headingDeg,
        headingConfidence: hasHeading ? 'high' : 'low', pitchDeg: hasHeading ? -24 : -18,
        fovDeg: hasHeading ? 56 : 44, rangeM: hasHeading ? 210 : 145, mountHeightM: hasHeading ? 10 : 8,
        groundElevationM: 150, feedType: 'image',
        ...
        sourceKind: 'austin-open-data', license: 'Public city traffic camera frame',
      });
    }
...
async function loadCaltransSourcesFromOpenData() {
  ...
  const districts = String(districtsRaw).split(',').map((t) => Number(t.trim())).filter((n) => Number.isInteger(n) && n >= 1 && n <= 12);
  if (!districts.length) return [];
  const settled = await Promise.allSettled(districts.map(async (district) => {
    const resp = await fetchWithTimeout(CALTRANS_CCTV_URL(district), { headers: { Accept: 'application/json' } }, CCTV_SOURCE_FETCH_TIMEOUT_MS);
  ...
  for (const result of settled) {
    ...
    for (const row of rows) {
      ...
      cameras.push({
        id: cameraId, name: loc.nearbyPlace ? `${label} (${loc.nearbyPlace})` : label,
        city: String(loc.nearbyPlace || `Caltrans D${district}`), cityId: `ca-d${district}`, provider: 'Caltrans',
        lat, lon, headingDeg: hasHeading ? heading : fallbackHeadingFromId(cameraId),
        headingConfidence: hasHeading ? 'high' : 'low', pitchDeg: hasHeading ? -24 : -18,
        fovDeg: hasHeading ? 56 : 44, rangeM: hasHeading ? 210 : 145, mountHeightM: hasHeading ? 10 : 8,
        groundElevationM: (() => { const ft = toFiniteNumber(loc.elevation, NaN); return Number.isFinite(ft) ? Math.max(-100, Math.min(4000, ft * 0.3048)) : 150; })(),
        feedType: 'image', url: imageUrl, snapshotUrl: imageUrl,
        sourceKind: 'caltrans-open-data', license: 'Public Caltrans highway camera frame',
      });
    }
  }
...
async function loadTflSourcesFromOpenData() {
  try {
    ...
    const resp = await fetchWithTimeout(url, { headers: { Accept: 'application/json' } }, CCTV_SOURCE_FETCH_TIMEOUT_MS);
    ...
    for (const place of places) {
      ...
      cameras.push({
        id: cameraId, name: String(place?.commonName || `JamCam ${rawId}`), city: 'London', cityId: 'london',
        provider: 'Transport for London', lat, lon, headingDeg: fallbackHeadingFromId(cameraId),
        headingConfidence: 'low', pitchDeg: -18, fovDeg: 44, rangeM: 145, mountHeightM: 8, groundElevationM: 15,
        feedType: 'image', url: imageUrl, snapshotUrl: imageUrl, sourceKind: 'tfl-open-data', license: 'Powered by TfL Open Data',
      });
    }
...
/** Assemble and KV-cache the merged CCTV source list (15 min fresh, serve-stale beyond). */
export async function getCctvSources() {
  const cached = await cache.get(CATALOG_KEY);
  const now = Date.now();
  if (cached && now - cached.at < CCTV_SOURCE_CACHE_TTL_S * 1000) return cached.sources;
  try {
    const sources = await refreshCctvSources();
    if (sources.length > 0 || !cached) {
      await cache.set(CATALOG_KEY, { at: now, sources }, 24 * 3600);
      return sources;
    }
    console.warn(`[CCTV] source refresh returned empty; serving ${cached.sources.length} stale cameras`);
    return cached.sources;
  } catch (error) {
    console.warn('[CCTV] source refresh failed:', error?.message || error);
    return cached?.sources || [];
  }
}
...
export function buildSyntheticCctvSvg({ cameraId, label, city, status }) {
  ...
  const ts = new Date().toISOString().replace('T', ' ').replace('Z', 'Z').slice(0, 20);
...
export async function proxyMediaResponse(res, upstream, { sourceHeader = 'upstream' } = {}) {
  ...
  const headers = { 'Content-Type': contentType, 'Cache-Control': cacheControl, 'X-CCTV-Source': sourceHeader };
...
export async function fetchCctvImageFromUpstream(url, { timeoutMs = CCTV_FRAME_FETCH_TIMEOUT_MS } = {}) {
  if (!url || !/^https?:\/\//i.test(url)) return null;
  try {
    const upstream = await fetchWithTimeout(url, { headers: { 'User-Agent': 'gods-eye-view-cctv-proxy/1.0' } }, timeoutMs);
...
// Best-effort, per-instance-memory health tracking (see file header note).
const health = new Map();
const HEALTH_MAX_ENTRIES = 1200;

export function setCctvHealth(cameraId, patch) {
...
export function listCctvHealth() {
...
export function buildStreamPayload(source, cameraId) {
  ...
  return {
    id: cameraId,
    feedType,
    mediaUrl: isVideoFeedType(feedType) ? `/api/cctv/media/${encodeURIComponent(cameraId)}` : null,
...
export async function streetViewFallback({ lat, lon, heading, fov, pitch }) {
  ...
  try {
    ...
    const svResp = await fetchWithTimeout(sv.toString(), { headers: { 'User-Agent': 'gods-eye-view-cctv-proxy/1.0' } }, CCTV_FRAME_FETCH_TIMEOUT_MS);
